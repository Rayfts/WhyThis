...
func (c *Client) PRFiles(ctx context.Context, repo Repo, number int) ([]PRFile, error) {
	...
	for page := 1; ; page++ {
		var batch []PRFile
		path := fmt.Sprintf("/repos/%s/%s/pulls/%d/files?per_page=%d&page=%d", url.PathEscape(repo.Owner), url.PathEscape(repo.Name), number, githubPageSize, page)
...
func (c *Client) PRCommits(ctx context.Context, repo Repo, number int) ([]PRCommit, error) {
	...
	for page := 1; ; page++ {
		var batch []PRCommit
		path := fmt.Sprintf("/repos/%s/%s/pulls/%d/commits?per_page=%d&page=%d", url.PathEscape(repo.Owner), url.PathEscape(repo.Name), number, githubPageSize, page)
		if err := c.get(ctx, path, &batch); err != nil {
			return nil, err
		}
		all = append(all, batch...)
