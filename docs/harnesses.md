...
| `aider` | `Aider-AI/aider` — `aider/website/docs/scripting.md` | `aider --message-file <file> --yes` | `--message-file` is documented as send/process/exit and `--yes` is the documented non-interactive confirmation flag. WhyThis runs synthesis in a temporary directory to isolate it from the analyzed repo. |
